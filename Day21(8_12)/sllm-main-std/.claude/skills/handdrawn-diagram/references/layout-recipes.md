...
실선은 실행 시점에 실제로 데이터가 흐르는 경로, 점선은 그렇지 않은 관계다.
같은 선으로 그리면 그림이 거짓말을 한다.

이 실습에서 갈리는 자리는 Evaluation Dataset 이다. Train 과 Validation 은 학습에
들어가지만 Evaluation 은 **학습에 쓰이지 않는다.** 학습 흐름에서 Evaluation 으로 가는
선을 실선으로 그으면 그 사실이 뒤집힌다. 학습을 마친 모델과 Base 모델 양쪽으로
갈라지는 평가 단계 화살표만 실선이고, 데이터셋에서 학습 쪽으로 가는 선과는 구분한다.
